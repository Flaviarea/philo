...
{
    if (av[1] == NULL || (is_digit(av[1]) == 0 || ft_atoi(av[1]) > PHILO_MAX) || 
        ft_atoi(av[1]) < PHILO_MIN || (ft_atoi(av[1]) <= 0))
        return (error_message("Invalid number of philosophers", 0));
    if (av[2] == NULL || is_digit(av[2]) == 0 || ft_atoi(av[2]) <= 0)
        return (error_message("Invalid time to die", 0));
    if (av[3] == NULL || is_digit(av[3]) == 0 || ft_atoi(av[3]) <= 0)
        return (error_message("Invalid time to eat", 0));
    if (av[4] == NULL || is_digit(av[4]) == 0 || ft_atoi(av[4]) <= 0)
