...
/*
	...
	Quindi, quando una funzione come is_digit() ritorna 0, vuol dire “nessun errore” → falso.
	Quando ritorna 1, vuol dire “errore” → vero.
*/

int check_args(int ac, char **av)
{
    if (!av[1] || is_digit(av[1])) // se is_digit è diverso da zero
    ...
    if (ac == 6)
    {
        if (!av[5] || is_digit(av[5]) || ft_atoi(av[5]) < 0)
